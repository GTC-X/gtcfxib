...
const SimpleForm = ({ activeTab }) => {
  ...
  const [initialValues, setInitialValues] = useState({
    ...
    "activeTab": "Liquidity Provider", // Dynamically includes the selected tab
    terms: false,
  });
  ...
  const formik = useFormik({
    initialValues: {
      ...initialValues,
      activeTab: activeTab || "", // Include activeTab in initial values
    },
    ...
    onSubmit: async (values) => {
      try {
        ...
        // Conditionally send the email based on activeTab
        if (activeTab === "Liquidity Provider") {
          const emailSmtp = await axios.post(
            `/api/register-email`,
            JSON.stringify({
              email: values.email,
          ...
          console.log("Email sent to Liquidity Provider user:", emailSmtp.data);
        } else if (activeTab === "Introductory Broker") {
          console.log("No email sent for Introducing Broker.");
        }
      ...
      <div className={`contact-form relative  rounded-lg shadow-lg overflow-hidden z-10 p-6 ltr:text-left rtl:text-right bg-gradient-to-b from-[#202d7bdb] via-[#050331] to-[#05033162]`}>
        ...
        <form onSubmit={formik.handleSubmit} className="py-3">
          ...
          <div>
            ...
            <div className="flex flex-col">
              <div className="flex items-start justify-start py-3 md:gap-1 ">
                <input
                  type="checkbox"
                  name="terms"
                  id="terms"
                  ...
                  className="h-5 w-5"
                />
                <p className="inline px-3 text-[9px] text-white">
              ...
              <label
                className={`text-[11px] ${formik.touched.terms && formik.errors.terms
                  ? "text-yellow-500"
                  : "text-white"
                  }`}
              >
